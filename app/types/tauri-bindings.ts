...
export const commands = {
  ...
  async getTrackData(pathString: string): Promise<Result<FileEntry, Error>> {
    try {
      return { data: await TAURI_INVOKE('get_track_data', { pathString }), status: 'ok' }
    }
    catch (e) {
      if (e instanceof Error)
        throw e
...
export interface FileEntry { path: string, name: string, filename: string, tags: Partial<{ [key in string]: string }>, thumbnail_uri: string, full_uri: string, is_playlist_track: boolean, valid: boolean, primary_tag: TagTypeArg | null }
